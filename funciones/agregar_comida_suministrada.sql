CREATE
OR REPLACE FUNCTION contabilidad_cerdos.agregar_comida_suministrada(
    in_user TEXT,
...
END IF;
    ...
    -- Validación que la cantidad del stock no sea menor que la suministrada
    IF
in_cantidad >(SELECT contenido_neto FROM contabilidad_cerdos.stock_productos WHERE producto_id = in_producto_id::INT) THEN
        RAISE EXCEPTION 'La cantidad de alimento suministrada es mayor a la que tiene en stock' USING ERRCODE = error_code_checked;
END IF;
...
success_message
:= 'El suministro de comida se registró correctamente';
