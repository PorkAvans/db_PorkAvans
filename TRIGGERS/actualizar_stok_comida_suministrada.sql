...
DECLARE
    ...
    cantidad_producto_actual NUMERIC;
    cantidad_producto_a_restar NUMERIC;
    nuevo_contenido_neto NUMERIC;
    tipo_producto INT;
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM contabilidad_cerdos.stock_productos WHERE producto_id = NEW.comida_suministrada_producto_id) THEN
        ...
        IF tipo_producto = 1 THEN
            ...
            nuevo_contenido_neto := (cantidad_producto_actual - cantidad_producto_a_restar) * contenido_unitario;

        ELSIF tipo_producto = 2 THEN
        ...
        WHERE producto_id = NEW.comida_suministrada_producto_id AND cantidad_producto < 0;

        UPDATE contabilidad_cerdos.stock_productos
        SET contenido_neto = 0
        WHERE producto_id = NEW.comida_suministrada_producto_id AND contenido_neto < 0;
    END IF;
...
CREATE TRIGGER trg_actualizar_stock_producto_suministrado
    AFTER INSERT ON contabilidad_cerdos.comida_suministrada
    FOR EACH ROW
